...
28. **THREE INCANT SPELLINGS THAT FAIL WITHOUT SAYING SO — CANDIDATE TRAPS, symptoms reproduced
    ...
    | `print "":;` for a blank line, **or any WHITESPACE-ONLY literal** | prints the **string `quoteBody`**. Use `print :;` (jiquery's idiom), or put real characters in the literal (`cerr "   rule " x`). ⚠ **RECLASSIFIED 2026-08-27 (ruled, Tony) — THIS ROW IS NOT A TRAP, IT IS DOCTRINE, AND IT IS THE ONE ROW HERE THAT NAMES NO DEFECT IN THE LANGUAGE.** **Kant has no empty strings, by design.** A literal with no characters carries no data, and a field with no data returns its own tag (#26) — that is the design working, not failing. **The defect is in the EXPECTATION**: code that writes `""` is asking for a value the language does not have, and gets the only honest answer there is. So there is nothing here to fix and nothing to guard against; there is a spelling to learn. ⚠ **WIDENED the same day: empty → EFFECTIVELY empty.** `cerr "   " x` prints `quoteBody` exactly as `""` does, and that is the form worth remembering, because the spelling reads as ordinary indentation and the output reads as a stray word rather than a missing one — so the expectation is wrong in a place nobody is looking at it. Same design, wider net |
