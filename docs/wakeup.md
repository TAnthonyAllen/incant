# ⚠⚠⚠ SEALED 2026-09-26, PAUSE -- THE parseMethod= ROAD AND THE C++ EMITTER ARE GONE, RETIRED BY MAPPING. RULINGS 1-3
# ARE OFF THE DOCKET; THE FLOOR CARRIES A GENERATED ROOT'S LABEL; RuleStuff.twk AND Generate.rtn ARE TIDIED.
#
#   ## THE ONE-LINE STATE: **trunk `jit-unified-emit-wip` installed and BARE -- fleet 753 / 1 · red 54 (banked by
#   name, docs/redList.md) · jitLadder 215 PASSED · printPop PASSED · canary 365 · fixit queue 1 · no incant
#   process. Branch `parse-then-fire` at a855e8a, pushed: PTF=0 753 / 1, PTF=1 752 / 1, jitLadder 215 at both,
#   canary 381.** Tony plans to return later today.
#
#   ## LANDED TODAY (each commit carries its certificate; ipc SEQ 182-192 / clod-to-clay SEQ 131-141)
#   - **Generate.rtn cleanup (SEQ 182):** six measures to measure.twk, one-line headers, alpha order, DesignDocs
#     reviewed; measure.twk sorted.
#   - **Ruling 1 -- the parseMethod= road retires by mapping; ruling 2 (P3a) joined it.** P3a's routing: treeOf
#     through driveStep (item A = incant/pop/driveDoorT, born red: door 1/1/0/0 and the file TRUNCATED by treeOf's
#     unfloored rejected pass). The READER by the drive floor: ParseActivation carries a label slot, checkInput
#     parks a generated DRIVE ROOT's label there (driveFloorLabel), driveStep hands it back, runRule maps it to one
#     bit, tell's reply reads it -- the root's rStuff.label was measured NULL after every drive (the call bracket
#     restores it; F-114's guard, untouched). The TREE ROW, incant/pop/treeRowT: generated tree AGREES with the
#     interpretive one on section 2.4's retag case. tree.sh and mixed.sh left the seal checklist.
#   - **Ruling 3a:** demoRprime retired; genScratch untruncated (sentinel, H2); rung7 went green before retiring.
#   - **The C++ emitter retired by mapping (SEQ 191), after its two kant-road successors went green:** shapeBodyT
#     (per-shape generated body + 16 drives per road) and parserCoverage (62 rules: 46 generate AND compile, 14
#     leaves, 2 no body -- ColoN, EquaL). Then 59 functions (the emitter, kant-door shims, stored-body verbs, tier 2's
#     binder, parseJSON*, the eight helpers), RuleStuff.termCount (layout, tokall), 7 setup registrations, 14
#     fixtures, 4 scripts, 10 targets, 47 DesignDocs entries; 20 rows retired, each with a dated successor line.
#   - **Task 2 (SEQ 192):** RuleStuff.twk tidied; parseGeneric deleted; subdirectories clean of the layout.
#
#   ## FINDINGS FOR TONY (reported, NOT minted -- rule F2)
#   - **A GENERATED BODY DOES NOT REPEAT A + REFERENCE.** shapeBodyT pins it: ShRep = ShA+ "c" on "aaac" -- old road
#     matched 1 consumed 4, generated 0/0. The other 15 drives agree. Candidate mechanism, unmeasured: the body calls
#     ShA() by name and reaches the rule, while + sits on ShRep's own term (dupTermRefusal's F-110 family).
#   - **countPop's 47 "missing" is its own instrument**: it scores f31's scaffold on the compile census, which is
#     silently dead (its counters went out with a comment relocation). parserCoverage does not read the census.
#   - Branch-only witnesses measureRetire, measureFireOrder, measureParseFire, measureKeywordDecision,
#     measureLabelReuse still live in Generate.rtn: moving them needs groups.ext lines trunk does not have -- owed
#     when the branch lands.
#
#   ## THE DOCKET
#   1. The step-2 plan resumes on the branch (P4 onward, docs/jitDesign.md): P3's harness question is ANSWERED (the
#      road retired; the tree row lives on the kant road), so re-read P3/P7 against today before building.
#   2. The ShRep repetition divergence -- yours to rule whether it is a defect to fix now.
#   3. Smaller: refireSkipsDegraded (fixit queue) · countPop's dead compile census · Limit removal (Tony) ·
#      genParse.rtn's dupCensus/dupTermRefusal out of alpha order (pre-existing).
#
#   ## CHECKLIST, measured at this tree (H12, H14) -- date checked, 2026-09-26 10:51
#   pop.sh 753 / 1 · jitLadder 215 ok, PASSED · decodePop FAILED, byte-identical to 09-25 · ddPop 5 / 1 ·
#   countPop FAILED, 0 crashed, 47 "missing" (unchanged) · printPop PASSED · frontier dies at station 4 (fire) ·
#   canary 365 · retok bare · groups.ext committed · Groups, support, TOK clean and pushed · no incant process.
#   (tree.sh and mixed.sh retired from this list 2026-09-26 -- successor treeRowT.)
#
