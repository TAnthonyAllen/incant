...
#   PASSED · printPop PASSED · canary 368 · fixit queue 2 · no incant process. Branch `parse-then-fire`, pushed:
#   PTF=1 740 / 2, PTF=0 742 / 2 (== trunk), jitLadder 215 at both, canary 383.**
#
#   ## THE DECISION: GO FOR STEP 2 (Tony, 2026-09-25). All three go/no-go conditions MET (ipc SEQ 123):
#   every divergence named and by design; the language plants still in the census's first two causes (today's
#   fixes are to step 1's own machinery); every channel's job has a step-2 home, no NONE row left.
#
#   ## RULINGS TODAY (Tony) -- each in its commit and in ipc SEQ 119-123
#   - **The yield channel (F-122 entry 7):** a construct hands its parent a VALUE; the label tree never holds a field,
#     a property node or a stand-in label; an unrun IF hands back nothing. **break / continue / return are CONTROL,
#     on a channel of their own** (the keyword action writes it, the firing parent reads it after each child).
#     Parse-deciding nulls stay exempt. All of it LANDS WITH STEP 2 -- nothing changed now.
#   - **A rejected parse fires nothing** (ruling (ii)): actions fire from a tree; the rejection is the report. The
#     exempt class stays only code that DECIDES the parse -- a diagnostic does not qualify.
#   - **The recording scope** (ruling A): a drive entered while firing is its own root. One fact (gPtfScope), one
#     writer pair, called from the two drive seats (driveStep, jitProbeDrive). The old road pushes nothing.
#   - **The "g"+tag stamp retired** (ruling C): zero live readers.
#   - **+=**: one method per kind, picked by pickKindOP on both roads; bin / data / members / empty(4a list -> string,
#     4b argument kind, 4c dataless -> structural). opPlusEQ is the refusal only. The members reopen '+=' directly
#     below its first line (setup section 9) -- a member block under an entry with a name=value attribute
#     overwrites that attribute (bear-trap #56).
#
#   ## MEASURED TODAY, worth keeping
#   - **Records never fired at PTF=1 are conserved exactly, 13339 either way**: scope off = FRAMELEAK 289 + unreachable
#     13050; scope on = FRAMELEAK 0 + rejected-parse discards 12407 + unreachable 932. The 289 was only the visible share.
#   - **nullAccessorDeref**, born fb9e4de (2026-09-08): seven accessors lawfully answer null and opDot's tail
#     dereferenced it. It was countPop's 47 crashes, f31's and bisectQ's. Guarded; nullAccessT pins all seven.
#   - **A bare accessor reads lastREF** -- in f31 and the repro, `if nexT` asked about the counter just referenced,
#     not the walked member.
#
#   ## THE DOCKET, IN ORDER
#   1. **Step 2 -- open with a PLAN AND RECON, not a build:** retire defer, deferredAbove and the yield channel; land
#      the value and control rulings; extend the scope beyond top-level statements; Rule C. Carry-ins: make the 932
#      failed-alternative records explicit discards; a one-door census of drive entries (probeDrive bypassing
#      driveStep was found by accident).
#   2. **The `.` operator:** getters per GroupField; opGet for subscripts (the shape-rewrite item). Later.
#   3. Smaller, open: refireSkipsDegraded (fixit queue) · kindSRT's jitted local surviving across fires · countPop's
#      47 probes now reading "missing" (no compile census) · f31's and bisectQ's new failures (fbSkipIt parse failed;
#      QuotE compile refusal) · the += member-binding trap as a citizen someday · decodePop red, unchanged since 09-24.
#   Parked, not advanced: parseLoop's silent success at max (F-114 entry 24); C-form.
#
#   ## CHECKLIST, measured at this tree (H12, H14) -- date checked, 2026-09-25 13:06
#   pop.sh 742 / 2 · jitLadder 215 ok, PASSED · decodePop FAILED, identical to this morning's capture ·
#   ddPop 5 / 1 · countPop FAILED -- 0 crashed, 47 "missing" (the crash is fixed; the census finding is open) ·
#   printPop PASSED · frontier dies at station 4 (fire) · canary 368 · retok bare · groups.ext committed ·
#   Groups, support, TOK clean and pushed · no incant process.
#
#   ## TONY'S FIXIT INCANTATIONS WAITING: **2** (oldest: nullAccessorDeref, since 2026-09-25)
#   Generated by `genLadder/fixitNag.sh`, not typed. ⚠ nullAccessorDeref is at REMEDY -- run it, bless the taken
#   signature, retire it.

# ⚠⚠⚠ SEALED 2026-09-25 -- += IS FINISHED: ONE METHOD PER KIND, PICKED PER FIRE ON BOTH ROADS, NO FALLBACK.
#
#   ## THE ONE-LINE STATE: **trunk `jit-unified-emit-wip`, installed and BARE -- fleet 725 / 2 · jitLadder 215 ok,
#   PASSED · printPop PASSED (NEW on the checklist) · canary 368 · fixit queue 1 (refireSkipsDegraded) · no incant
#   process.** The branch `parse-then-fire` is untouched; its go/no-go is still open.
#
#   ## ⚠ CORRECTION OWED BY EARLIER SEALS, recorded here and NOT by rewriting them: **seals 2026-09-11 through
#   2026-09-24 recorded countPop as "47 of 47 attempted, foot reached" while EVERY PROBE CRASHED.** 131253e moved
#   incant/f31 to incant/pop/f31 and genLadder/mkProbeOne.py kept opening the old path; the line was true and
#   described nothing. countPop now counts COMPLETED probes and fails on any crash. With the path repaired, all 47
#   probes segfault in opDot <- runOP <- aCTionIF inside a while -- dating that crash is the next dispatch.
#
#   ## WHAT LANDED TODAY (Tony's rulings, each commit carries its certificate)
#   - **checkOP** (Tony, offline) seated in runOP; **getDataType returns the kind NAME** through dataName (ONE table).
#   - **pickKindOP is the whole += pick**, one place, both roads: bin -> structural; data -> its kind; no data +
#     members (rules too: the grammar graft) -> structural; EMPTY -> 4a list argument -> the string member (35a),
#     4b argument data -> its kind, 4c dataless node -> structural. Structural is hasMembers, never groupList.
#   - **Members of '+='**: isCOUNT, isNUMBER, isSTRING (also isTOKEN; absorbs list-concat), isBUFFER, isSTAK, struct.
#     Each is its arm lifted; bad argument shapes refuse through refuse(). **opPlusEQ holds only the named refusal.**
#   - **The jitted road fires the pick at RUN time** (jitOpFireRT): a cursor over mixed kinds, a target typed by its
#     first fire, a holder refusing per fire -- all agree with the interpreted road; degrade 0 on every kind fixture.
#   - The frame-slot KIND gap closed (gJitFrameAssigned): a jitted local assigned by `=` then += reads right.
#   - printPop re-pinned (Tony removed its two trace prints 08-18, 4ab72dd) and **joined the seal checklist**.
#   - jitEmitStringPlusEQ and concatEQ deleted (no caller once opPlusEQ lost its string arm) -- canary 370 -> 368.
#
#   ## FINDINGS -- the first one cost the most
#   - **Bear-trap #56, rewritten twice today:** a member block under an entry with a name=value attribute CORRUPTS
#     that attribute. '+=' was bound to opPlusEQisCOUNT from 2174588 until batch 0, MASKED because the old count
#     member handed every other kind back to opPlusEQ and its witness sat after the hand-back. The members now
#     reopen '+=' as the LAST entry of the Operators define (setup section 9; Tony's simplification).
#   - **A jitted action LOCAL keeps its value across fires** where the interpreted road clears it, += or not
#     (`ctL = ctL + 1`: 1 2 3 jitted, 0 0 0 interpreted). Pinned as a divergence in kindSRT. Frame model.
#   - **A refire silently skips every statement that DEGRADED at compile** (cerr, search; FOR is re-run). Banked as
#     incant/fixits/refireSkipsDegraded, NEXT: OPEN.
#   - **The call-through is gated on '+=' HAVING MEMBERS** -- a fact of setup registration. opPlusEQ keeps one
#     `if jitting` degrade for that case. Tony's to rule whether the gate should be a property of the op.
#
#   ## NEXT: date the countPop crash -- minimal repro first, a good end at or before 131253e, then a three-repo
#   bisect in a clone outside Dropbox. Then the parse-then-fire go/no-go.
#
#   ## CHECKLIST, measured at this tree (H12, H14) -- date checked, 2026-09-25 11:11
#   pop.sh 725 / 2 · jitLadder 215 ok, PASSED · decodePop FAILED, the same 5 rows as 09-24 · ddPop 5 / 1 ·
#   **countPop FAILED -- 47 of 47 probes crashed (the repaired instrument; see the correction above)** ·
#   **printPop PASSED** · frontier dies at station 4 (fire) · canary 368 · retok bare · groups.ext committed ·
#   Groups, support, TOK clean and pushed · no incant process.
#
#   ## TONY'S FIXIT INCANTATIONS WAITING: **1** (oldest: refireSkipsDegraded, since 2026-09-25)
