...
| **writer, guard, accessor** | ⚠ **CORRECTED 2026-09-20 — `ruleActions.rtn:446` IS NOT A SETTER, AND `isArgument` IS NOT WRITTEN IN `ruleActions.rtn` AT ALL.** Measured whole-tree: the single writer is **`GroupActions.rtn:883`**, runAction's mint. It was briefly **two** — `Generate.rtn:213` minted a slot onto the RULE — and that second writer was **deleted the same day** when Tony ruled a rule carries no `argument`. **§2.3's seven-readers-in-three-classes sizing still rests on the stale line and is owed a re-run before anything is built on it.** · `ruleActions.rtn:897` · `Instruct.rtn:340` case 10 (an `isArgument` property exposed to incant) | **SURVIVE untouched** |
