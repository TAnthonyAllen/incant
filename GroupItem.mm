...
{
	// twoRoads a NEW-road fire walks the activation list (who is active above) to the drive floor, skipping itself; an OLD-road fire keeps the parentStuff walk (where it attaches) -- ruling (a), Tony 2026-09-24
	
	int newRoad = gFireFromNewRoad;
	gFireFromNewRoad = 0;
	int inDrive = gParseActive && !gParseActive->floor;
	if ( GroupControl::groupController->groupRules->processingCode )
	{ ::measureDeferredAbove(stuff,newRoad,1,3,inDrive); return 1; }
	if ( newRoad )
	{
	ParseActivation *a = gParseActive;
	if ( a && !a->floor && a->stuff == stuff ) a = a->prev;
...
***************************************************************************/
void GroupItem::setDebug()
{
GroupItem 	*grup = 0;
int 		debugStatus = groupBody->flags.debugged;
	while ( grup = next(grup) )
		{
		if ( grup->groupBody->flags.noPrint )
			continue;
		if ( grup->groupBody->flags.debugged == debugStatus )
			continue;
