...
/***************************************************************************
    ...
    IT SUPPLIES THE PRODUCT, not merely a side record. The setToken at the foot
    is the channel every consumer already reads; the noPrint attribute above it
    is kept as free debug surface and nothing consults it. Ruled 2026-08-23.

    THE TRACE READS THE PRODUCT BEFORE THE WRITE, deliberately. Once this
    function supplies the product, a comparison taken AFTER the write would
    compare this function to itself and report MATCH forever -- vacuity in a
    new coat. The oracle is sampled first or it is not an oracle.
...
/***************************************************************************
    ...
    parse()'s fork used to carry this write inline, under a comment reading
    "THIS LINE IS ITS SINGLE WRITER". That was true and it is still true: the
    doctrine is one BODY, not one call site. Both seams call here.

    ⚠ WHY runRule NEEDED IT. The new parse road dispatches
    `newParse.method(rule)` straight at builtinParsE and never calls
    rule.parse(), so it never crossed the writing line. Measured 2026-08-29:
    PARENTPROBE printed parentLabel=0x0 on all four invocations of an Xpress
    walk -- Xpress, ExpressioN, Token, QuotE -- while the structural parent
    named definitional positions (Token's own parent is Token). The frame
    channel existed and the new road did not write it.

    ⚠⚠ THE DISCIPLINE IS NOT SAVE/RESTORE, AND THAT IS WORTH KNOWING BEFORE
    ANYONE "FIXES" IT. There is no save and no restore here, and the incumbent
    had none either -- it is a bare write immediately before the call. The
    recursion safety lives in the CALLEE: parse()'s fork comment states that
    the callee lifts parentLabel into a stack local at entry before descending,
    so a nested invocation overwriting this slot cannot disturb an outer frame
    already under way. The genParse-emitted methods implement exactly that --
    `GroupItem into = rule.rStuff.parentLabel;` is their first line.

    THE CONSEQUENCE, NAMED RATHER THAN LEFT TO BITE: a parseMethod that reads
    parentLabel LATE instead of lifting it at entry is unsafe under recursion,
    and parseRule does not lift today because it does not yet read the frame at
    all. When setParentLabel lands, the lift is owed AT parseRule's ENTRY, not
    at the point of use.
***************************************************************************/
...
/***************************************************************************
    frameParent -- resolve the parse-time parent for a rule invoked through
    runRule, which has no pStuff to read one from.

    IT READS runRule's OWN ARGUMENT, and that is the whole finding. The holder
    is the node the call site handed down -- the InvokeArg for a sub-rule call,
    the argument field for a top-level one -- and its rStuff carries a label
    that is DISTINCT PER INVOCATION. So the caller's name reaches the writer as
    dataflow along the existing call, with no emitter change and no channel
    invented.

    ⚠ THE FIRST VERSION READ ruler->ruleSTUFF AND WAS WRONG, MEASURABLY.
    A singleton is not a channel: a distant read of shared state answers "what
    happened last", never "who is asking". It printed ONE pointer for all four
    invocations of an Xpress walk -- the driver action's own BlocK, stale --
    where four distinct ones were pre-registered. The holder's own stuff prints
    four distinct labels on the same run, and the singleton prints its single
    stale pointer beside them, which is the control.

    A top-level invocation's holder has no rStuff at all (the probe shows
    fieldStuff=0x0 for ruleText), so it falls through to the holder itself --
    the argument holder, which is what a top-level parse should attach into.

    Separate from establishFrame deliberately: runRule is a declared-field
    function and after the parkOnMaster near-miss no new declarations go inside
    one. A call introduces no declaration, so the resolution happens here and
    runRule's own bare-name resolution cannot move.
***************************************************************************/
