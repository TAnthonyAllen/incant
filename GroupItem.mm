...
/***************************************************************************
    ...
    An invocation reached from inside another rule's generated body carries the
    invoking rule's stuff in the rules singleton, so its label is the in-flight
    parent. A top-level invocation has none, and its parent is the argument
    holder the caller diverted input from.
...
{
GroupRules 	*ruler = GroupControl::groupController->groupRules;
	if ( ruler->ruleSTUFF && ruler->ruleSTUFF->label )
		return ruler->ruleSTUFF->label;
	return holder;
}
