# incant/attic — A STAGING AREA FOR DELETION, NOT A MUSEUM

**Tony's ruling, 2026-08-28.** Anything in here is **one step from deletion**. This is the
staging area for the shitcan, not storage.

- **A file that sits here unmissed across seal cycles gets deleted without further ceremony.**
  Nothing needs to justify removing it. Silence is the verdict.
- **Restoration requires a stated reason.** Moving a file back out is a decision someone signs,
  not a default.
- **Nothing in here is run, included, or cited by any instrument.** That is what put it here: the
  trim census found no runner, no document, and no `CLAUDE.md`/`TODO.md` row naming it.

The repo is the archive — history holds every one of these files, so the attic is not the copy of
last resort and must never be treated as one.
