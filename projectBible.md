...
- **`=` (opAssign)** — value-copy. Sets A's content to B's content, but **keeps A's own tag** (tag-imposition — B's tag does not transfer). A and B remain distinct.
- **`:=` (opSetGroup)** — group-binding / reference. Binds A to *reference* B's node rather than copying content out. Two verified consequences (2026-06): **(a) the argument's tag survives** — `endLabel := new("bcLabel1")` yields a node still tagged `bcLabel1`, where `endLabel = new("bcLabel1")` (with `=`) would re-tag it `endLabel`; **(b) `:=` stamps a sticky `byRef` flag on the argument** — any *later* `=` on that same field then also references instead of copying. The stickiness is a known audit item: `:=` sites whose fields are later legitimately `=`-copied would alias (see TODO).
...
### Name Resolution and Base Registries
Bare references in incant code (writing `getFile` or `'>'` or `bcCALL`) resolve through a registry walk. Resolution order:

1. **Current registry** (set globally via `register(name)` or `define name registry ...;`)
2. **searchList** (user-controllable working set, maintained via the `search` command — e.g., `search reset stack Grokking Generating Stuff list;`)
3. **Base registries** (always searched; six entries set in C++):
   - `properties` (pROPERTIEs) — mirrors of GroupRules fields, accessible from incant
   - `opFields` — operators (also referenced as `Operators` in incant source; same registry, two names)
   - `commands` — command-style primitives like getFile, defer, isList
   - `files` — file-related field registry
   - `keyWords` — parse-reserved names
   - `groupFields` — field-name registry

The base-registry property is what makes bare-name lookup universal — anywhere in incant code, `getFile(filename)` works without qualification. The lookup mechanism lives in aCTionNamE (ruleActions.rtn:388) via `locateInMethod()`.

For operators with non-alpha names, single quotes are used to look up the operator as data: `field = '>'` sets field to the `>` operator. `field = '+'` without quotes would invoke the operator. Single-quote form means "treat this as a name lookup, not as the operator itself in expression position."

currentRegistry is a global; both setters (`register()` and `define name registry`) set it for the rest of the file or session until changed again. No automatic scoping.

### Attributes with Methods — Two Patterns
Any attribute can have a method. How the method is used distinguishes two patterns:

**Setter-attribute pattern.** Attribute has `noPrint=true` and `immediateACTION=true`. When aCTionDefinE (ruleActions.rtn:137) processes a definition's attributes, it dispatches setter-attributes through their method — running the method with the defining-field as context, with `fLAG=true` so the method knows it's being called from a definition. After running, the attribute itself is discarded. The method's purpose was to set state on the parent field. Example: `operateMethod=opGT` in an Operators entry sets `op.method = opGT` and disappears; the op itself becomes callable as `op.method(...)`.

**Persistent attribute with method.** Method exists on the attribute, but doesn't fire automatically. To use it, the attribute is accessed and its method called explicitly: `attribute.method(arg)` or `attribute.method(attribute)`. Example: `interpret` sub-attribute on bcOPs entries (if it has a method) is invoked explicitly by the bytecode interpret() loop rather than firing during definition.

The discriminator is `noPrint && immediateACTION`. With both flags set, the attribute is a setter and is discarded after firing. Without them, the attribute persists with whatever method it has.

This distinction matters when reading registry definitions — a definition that looks like `field someAttr=someValue` may either (a) configure the field at definition time and disappear (setter), or (b) leave someAttr as a sub-attribute available for later use (persistent). Knowing which is which requires either looking at the source definition's attribute declarations (does someAttr carry noPrint+immediateACTION?) or knowing the pattern from project conventions.
...
15. **Incant directives are non-idempotent** — unlike tok directives, `replaceAt`/`insertAt` applied twice to the same file double the effect. Always apply to a copy, never the source. Working pattern: copy target into `Groups/Tests/` (not a symlink), run incant against the copy, diff copy against original to verify. Tests/ copies are gitignored scratch. (2026-06-15)
16. **`NamE` rule requires `^` on `nameSet`** — without the `^` noSkip modifier, the rule greedily consumes whitespace-separated tokens as a single name (`x code` parsed as one name). Correct rule: `NamE first-=[a-zA-Z] nameSet-^* tokenize;`. The `^` before `*` is load-bearing. (2026-06-15)
...
- **Frontier (updated per `docs/tawk-tok-compile-frontier.md`, 2026-05-31; full plan in `docs/tawk-replacement-plan.md`)**: the original `generateRules` **class-body/extern-split blocker** (class field decls like `currentClass` escaping the `class Tawk extends PLGparse {…}` wrapper → tok `ERROR Inheritance`) is **SOLVED** — the per-alternative *named-options* rework (Alternative carries the action; generateRules distributes the numbered `.act` shells onto a rule's alternatives) made the parse 100% clean (0 `ERROR Inheritance`). The **live blocker is now one layer down**: `tok <regen>` parses clean but **crashes in codegen** partway through stale action bodies still on the old-PLGitem surface (e.g. `hash[PLGitem]` segfaults tok; removed fields like `type.flag4`/`guardSet`/`currentTest`). Remaining work = a method-by-method reconciliation of the 5 Tokf `.act` action bodies — a mechanical grind, mappings tracked in the frontier doc. Until it completes, **`Tokf/Tawk.twk` stays the hand-maintained legacy file** (commit `89a3abc`, old format, toks) — NOT the regen; run all regen work from `Tokf/Tests/`. Broken intermediates to avoid: `ef2730d` (new-format setRules with C++ `elem->` arrows tok can't parse) and any fresh `plg Tawk.g` overwrite of the legacy source.
*(Phase Generate Tawk detail + regen contract folded from Parse's projectBible.md 2026-06-10; blocker status corrected against the 2026-05-31 compile-frontier doc — F5.)*
...
- **Incant directives: `replaceAt` / `insertAt` landed (2026-06-15).** Text-substrate replace and before/after-line insert, better than tok directives on two axes: mid-line content match (not just line-start) and arbitrary block size. Tested via `dIRECTive1/2/3` fixtures in `IncantForms/Windows/tabs`; to be moved into `incant/directives` with docs. Two known limits: (a) non-idempotent — apply to a copy (see Key Design Decision #15); (b) comment matching requires a `checkSkip()` change — currently swallows anything that looks like a comment even inside quotes.
...
### Incant Working ✅ [updated 2026-06-15]
...
- `replaceAt` / `insertAt` directive actions — text-substrate replace and before/after-line insert ✅ (2026-06-15); `setMark` promoted to incant command; `getMarkLineAt` added
- `Buffer.twk` — `appendString`/`appendChar` formatted variants + `shiftAtMark` ✅ (2026-06-15)
