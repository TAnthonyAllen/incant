...
GroupItem 	*source = 0;
	...
	Two of C-148's pre-registered outcomes collide on bare and cannot both
	hold without this line: "unWrap calls go / never follows" and "the 73
	bare-argument sites UNMOVED on main". On bare, gNoUnwrap is 0 and the
	action's `argument` IS a holder -- measured at SEQ 145, ANYtoken ->
	argument -> sample, two hops -- so a source that never follows refuses
	on every one of those 73 sites the moment it lands.
	So the PARSE hop is peeled unconditionally above (that is C-146's
	lesson, and gating it is what broke every iterate), and the HOLDER
	follow is gated on the migration switch exactly as its own header says
	it should be: 0 LEGACY auto-unwrap live, 1 FLIPPED no auto-unwrap.
	⚠ THIS ARM RETIRES WITH gNoUnwrap, in the same stroke, per that
	switch's pre-registered obituary. It is not a second mechanism.
	ruleActions.aCTionIterate.legacyFollow  */
