...
/*****************************************************************************
     ...
     ⚠⚠ ANY DECLARATION ADDED INSIDE THIS FUNCTION OWES THE BEAR-TRAP 42 CHECK,
     AND IT IS A NAMED CHECK RATHER THAN A HABIT: tok resolves bare fields
     last-mentioned-wins, so a new local RE-POINTS EVERY BARE FIELD BELOW IT AND
     COMPILES CLEAN. It bit here on 2026-09-09 -- `checkInput()` aimed at
     `intoStuff` and the locals-clear loop cleared `ruleArg` instead of `grup`,
     silently. The `use ruleStuff` / `use grup` lines below are the cure. THE
     CHECK, one command, after every retok and BEFORE the build:

        diff <(git show HEAD:GroupRules.mm | awk '/^extern "C" GroupItem \*parseRule/{f=1} f{print} f&&/^}/{exit}' \
                 | grep -E "checkInput|isLocal|grup->clear|atRuleMark = from") \
             <(awk '/^extern "C" GroupItem \*parseRule/{f=1} f{print} f&&/^}/{exit}' GroupRules.mm \
                 | grep -E "checkInput|isLocal|grup->clear|atRuleMark = from")

     The extern canary CANNOT see this one -- it read 318 throughout.
     parseRule.bareFieldRepoint carries the argument.
*****************************************************************************/
...
int 		yielded = 0;
	...
	if ( ruleStuff->checkInput() )
		...
		/*  parseRule.chainTruthOnly  RULING (c'), Tony, 2026-09-09. The return
		carries ONE BIT -- the AND chain's truth -- and never a label. This is
		leaveRule's shape: RuleStuff.twk:776 branches on `ok` and nothing
		else, and `ok` is the chain (genParse.rtn:596). Measured H20: the
		chain hands back trueResult on a match and falseResult BY IDENTITY on
		a miss, and truthOf reads them 1 and 0 -- so the bit is real and a
		false chain is genuinely falsy, not F-55's sixth face.
		⚠ A TRUTH VALUE CARRIES NO METHOD (H16), so bear-trap 34 cannot reach
		this return. ruleAsLabel stays anyway, now as a GENERATOR-ERROR guard:
		a label in return position means the emitter is wrong, not the input.
		⚠ OBSTACLE 3 IS RETIRED STRUCTURALLY RATHER THAN GUARDED. The empty
		label was this road's failure case AND its crash case because
		listLength decided the yield; under (c') truthOf decides, so the
		deref is simply GONE. Unconstructable beats guarded.  */
		yielded = 0;
