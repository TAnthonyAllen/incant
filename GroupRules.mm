...
extern "C" GroupItem *ptfStatementEnd(GroupItem *field, RuleStuff *stuff)
{
	...
	if ( ptfTraceOn() && isGROUP(ret->groupBody->flags.data) && r->max > 1 )
	::fprintf(stderr,"PTF LATEGROUP rule=%s -- trunk attaches this label's group, not the label\n",r->rule->groupBody->tag);
	GroupItem *put = pa->addAttribute(ret);
	if ( !put ) continue;
	put->remove();
